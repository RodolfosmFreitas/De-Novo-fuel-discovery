...
data    = pd.read_excel('{}/data_YSI.xlsx'.format(args.data_dir)).to_numpy() 
fuel_list = data[:,0]
SMILES = data[:,1].tolist()

# load the Unified Sooting Index dataset
Y_pure = data[:,4].astype(np.float64)[:,None]
...
# read fuel data [Name, SMILES, CN, METHOD]
data_blends    = pd.read_excel('{}/data_blends_YSI.xlsx'.format(args.data_dir))
fuel_list_blends = data_blends['Mixture Name'].to_numpy()
SMILES_blends = list(data_blends.keys()[1:-3])

# load the cetane numbers 
Y_blends = data_blends['Unified YSI'].to_numpy()[:,None]
...
torch.save(mu, model_dir + "/data_YSI_stats_mean.pt")
torch.save(std, model_dir + "/data_YSI_stats_std.pt")
torch.save(X_train, model_dir + "/train_x_YSI.pt")
torch.save(Y_train, model_dir + "/train_y_YSI.pt")
#%% ================================ Training ===================================#
...
# save the model
torch.save(model.state_dict(), model_dir + "/DKL_model_YSI.pt")
...
plt.savefig(model_dir+'/marginal_log_likelihood_YSI.jpg')
plt.show()
...
var_pred_train = Y_pred_train.variance.detach().cpu().numpy() * std + mu   

# "Diagnostics: How do you know if the fit is good?"

mape_train = mean_absolute_percentage_error(Y_train, mu_pred_train)
mae_train = mean_absolute_error(Y_train, mu_pred_train) / np.mean(Y_train)
...
var_pred_test = Y_pred_test.variance.detach().cpu().numpy() * std + mu    

# "Diagnostics: How do you know if the fit is good?"

mape_test = mean_absolute_percentage_error(Y_test, mu_pred_test)
mae_test = mean_absolute_error(Y_test, mu_pred_test) / np.mean(Y_test)
...
plt.scatter(np.reshape(mu_pred_train,-1),np.reshape(Y_train,-1), 
                  s=np.sqrt(var_pred_train) ,marker='s', color = colors[0],  alpha=0.5, label=r'Train')
acc_train = (f'R$^2$ = {r2_train:.3f}\n'
        f'nMAE={mae_train:.3f}')

bbox = dict(boxstyle='round', fc=colors[0], ec=colors[0], alpha=0.15)
plt.text(1200., 150, acc_train, bbox=bbox, horizontalalignment='right')


plt.scatter(np.reshape(mu_pred_test,-1),np.reshape(Y_test,-1), 
                  s=np.sqrt(var_pred_test) ,marker='o', color = colors[1], alpha=0.75, label=r'Test')
acc_test = (f'R$^2$ = {r2_test:.3f}\n'
        f'nMAE={mae_test:.3f}')
bbox = dict(boxstyle='round', fc=colors[1], ec=colors[1], alpha=0.15)
plt.text(1200., 350, acc_test, bbox=bbox, horizontalalignment='right')
...
ticks = np.linspace(0, 1400, 6)
plt.xticks(ticks)
plt.yticks(ticks)
plt.legend(loc='best', frameon=False, prop={'weight': 'extra bold'})
plt.savefig(model_dir + '/parity_YSI.pdf', bbox_inches='tight')
plt.show()
...
plt.xlim([-10,10])
plt.legend(loc='best', frameon=False, prop={'weight': 'extra bold'})
plt.savefig(model_dir + '/PDF_error_YSI.pdf', bbox_inches='tight')
plt.show()
...
plt.yticks(ticks)
plt.legend(loc='best', frameon=False, prop={'weight': 'extra bold'})
plt.savefig(model_dir + '/parity_YSI_test_pure_blends.pdf', bbox_inches='tight')
plt.show()
...
acc_train = (f'R$^2$ = {r2_train:.3f}\n'
        f'nMAE={mae_train:.3f}')
bbox = dict(boxstyle='round', fc=colors[0], ec=colors[0], alpha=0.15)
axes[0].text(1500., 10, acc_train, bbox=bbox, horizontalalignment='right')
#
...
acc_test = (f'R$^2$ = {r2_test:.3f}\n'
        f'nMAE={mae_test:.3f}')
bbox = dict(boxstyle='round', fc=colors[1], ec=colors[1], alpha=0.15)
axes[0].text(1500., 350, acc_test, bbox=bbox, horizontalalignment='right')
#
min_data  = np.minimum(np.minimum(np.amin(Y_train),np.amin(mu_pred_train)), np.minimum(np.amin(Y_test), np.amin(mu_pred_test)))
max_data = np.maximum(np.maximum(np.amax(Y_train),np.amax(mu_pred_train)), np.maximum(np.amax(Y_test),np.amax((mu_pred_test))))
xlim = plt.xlim(min_data, max_data)
...
ticks = np.linspace(0, 1400, 3)
plt.xticks(ticks)
plt.yticks(ticks)
axes[0].set_xlabel(r'Predicted YSI')
axes[0].set_ylabel(r'Measured YSI')
...
axes[2].set_xlabel(r'Predicted YSI')
axes[2].set_ylabel(r'Measured YSI')
...
# Save figure
plt.savefig(model_dir + "/YSI_panel.pdf", bbox_inches='tight')
plt.show()
...
plt.ylabel(r'Uncertainty - YSI')
plt.xlabel(r'Error - YSI')
plt.savefig(model_dir +'/Error_uncertainty_YSI.pdf', bbox_inches='tight')
plt.show()